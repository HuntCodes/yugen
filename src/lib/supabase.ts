...
export async function testAuth() {
  try {
    ...
    const { user: signUpUser, session: signUpSession, error: signUpError } = await supabase.auth.signUp({
      email: testEmail,
    ...
    const { user: signInUser, session: signInSession, error: signInError } = await supabase.auth.signIn({
      email: testEmail,
      password: testPassword,
    });
