...
export function getSuggestedShoe(sessionType: string): string | null {
  ...
  if (type.includes('easy run') || 
      type.includes('progression run') || 
      type === 'strides') {
    return 'Cloudmonster';
  }
  
  // Cloudeclipse for long run, strength training
  if (type.includes('long run') || 
      type.includes('strength training')) {
    return 'Cloudeclipse';
  }
  
  // Cloudboom Zone for tempo run, hills, fartlek, strides
  if (type.includes('tempo run') || 
      type.includes('hills') || 
