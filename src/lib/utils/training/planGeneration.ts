...
export function generateFallbackPlan(daysPerWeek: number = 3, weeklyVolume: number = 20, units: string = 'km'): TrainingSession[] {
  ...
  console.log(`[Fallback] Sessions per week: ${sessionsPerWeek}, Avg session distance: ${avgSessionDistance.toFixed(1)} ${units}`);
  ...
  const createSession = (date: Date, dayNumber: number, weekNumber: number, sessionType: string, distanceMultiplier: number = 1, timeOfDay?: string): TrainingSession => {
  ...
  for (let i = currentDay; i <= 7; i++) {
    ...
    if (trainingDays.includes(dayNumber === 7 ? 0 : dayNumber)) {
      ...
      if (daysPerWeek <= 3) {
        ...
        if (dayNumber === 3 || dayNumber === 4) { // Wed/Thu
          sessionType = "Tempo Run";
          distanceMultiplier = 0.8;
        } else if (dayNumber === 6 || dayNumber === 7) { // Sat/Sun
          sessionType = "Long Run";
          distanceMultiplier = 1.5;
        }
      } else if (daysPerWeek <= 5) {
        // Medium frequency: add variety
        if (dayNumber === 2) { // Tuesday
          sessionType = "Easy Run + Strides";
          distanceMultiplier = 1.0;
        } else if (dayNumber === 4) { // Thursday
          sessionType = "Tempo Run";
          distanceMultiplier = 0.8;
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (trainingDays.includes(i === 7 ? 0 : i)) {
      ...
      if (needsDoubleDays && i !== 7) { // Not Sunday
  ...
  console.log(`[Fallback] Week 2 volume: ${week2Volume.toFixed(1)} ${units} (target: ${weeklyVolume}), sessions: ${week2Sessions.length}`);
    ...
    if (trainingDays.includes(dayOfWeek === 7 ? 0 : dayOfWeek)) {
      ...
      if (phase === "Base") {
        if (dayOfWeek === 2) { // Tuesday
          ...
          sessionType = "Long Run";
        } else if (dayOfWeek === 3 && daysPerWeek >= 5) { // Wednesday for higher frequency
          sessionType = "Fartlek";
        } else if (dayOfWeek === 5 && daysPerWeek >= 6) { // Friday for high frequency
          sessionType = "Recovery Run";
        }
      } else if (phase === "Build") {
        if (dayOfWeek === 2) { // Tuesday
          ...
          sessionType = "Long Run";
        } else if (dayOfWeek === 3 && daysPerWeek >= 5) { // Wednesday
          sessionType = "Hill Repeats";
        } else if (dayOfWeek === 5 && daysPerWeek >= 6) { // Friday
          sessionType = "Easy Run + Strides";
        }
      } else if (phase === "Peak") {
        if (dayOfWeek === 2) { // Tuesday
