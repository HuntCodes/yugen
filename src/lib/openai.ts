        ...
        try {
          ...
          if (targetVolume > 0) {
            ...
            if (week2Volume < targetVolume * 0.85) {
              console.warn(`[OpenAI] ⚠️ Generated volume is significantly below target (${week2Volume} vs ${targetVolume})`);
            }
            
            // Log double days if any
            const week2DoubleDays = week2Sessions.reduce((acc, session) => {
              acc[session.date] = (acc[session.date] || 0) + 1;
              return acc;
            }, {} as Record<string, number>);
            
            const doubleDayCount = Object.values(week2DoubleDays).filter(count => count > 1).length;
