...
export async function generateWeeklyPlan(
  ...
  const weeksSincePlanStart = Math.floor(
    (nextMonday.getTime() - planStartDate.getTime()) / (7 * 24 * 60 * 60 * 1000)
  );
  ...
  const currentMileage =
    userProfile?.weekly_volume || onboardingData.current_mileage || 'Not specified';
  ...
  const { trainingPreferencesSection, struggleSection, feedbackSection } = prepareTrainingFeedback(
    trainingFeedback,
    userProfile,
    onboardingData
  );
