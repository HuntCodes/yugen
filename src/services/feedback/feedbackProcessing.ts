...
export async function processWeeklyTrainingFeedback(
  ...
  try {
    ...
    console.log(
      `Processing training feedback for user ${userId} for week ${startDateStr} to ${endDateStr}`
    );
...
export async function processAllUsersFeedback(): Promise<{
  ...
  try {
    ...
    console.log(
      `Training feedback processing complete. Processed: ${result.processed}, Failed: ${result.failed}`
    );
    return result;
  } catch (error) {
