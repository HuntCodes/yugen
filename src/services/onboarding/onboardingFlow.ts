...
export async function handleOnboardingConversation(
  ...
  let conversationHistory = [...currentContext.conversationHistory];
  ...
  try {
    ...
    const messages: any[] = [ // Using any for now, will refine based on OpenAI library types if available
    ...
    if (choice.message.tool_calls && choice.message.tool_calls.length > 0) {
      ...
      if (toolCall.function.name === "update_onboarding_profile") {
        ...
        coachMessage = choice.message.content || "Okay, I've processed that information."; 
        isCompleteByToolCall = true;
        console.log("[handleOnboardingConversation] AI called 'update_onboarding_profile'. Arguments:", toolCallArguments);
        console.log("[handleOnboardingConversation] Coach message accompanying tool call:", coachMessage);
      } else {
      ...
      coachMessage = choice.message.content ? choice.message.content.trim() : "I'm not sure what to say.";
    }
    ...
    const isCompleteByPhrase = completionPhrases.some(phrase => lowerCoachMessage.includes(phrase));
    ...
    if (isCompleteByToolCall && isCompleteByPhrase) {
      console.log("[handleOnboardingConversation] Onboarding complete by BOTH tool call and completion phrase.");
    ...
    return {
      ...
      isComplete,
      conversationHistory,
      toolCallArguments: toolCallArguments, // Include this in the result
    };
  } catch (error) {
...
const updateOnboardingProfileTool = {
  ...
  function: {
    ...
    description: "Updates the user's profile with information gathered during the onboarding conversation. Call this function once all required information has been collected.",
    parameters: {
      ...
      properties: {
        ...
        current_mileage: {
          type: "string" as const,
          description: "The athlete's current weekly running mileage (e.g., '30', '25-35'). Include the unit if specified, otherwise assume based on 'units' field."
        },
        current_frequency: {
          type: "string" as const,
          description: "How many days per week the athlete currently runs. This should be a numerical string (e.g., '3', '5', '7' for everyday)."
        },
        experience_level: {
          type: "string" as const,
        ...
        goal_type: {
          type: "string" as const,
          description: "The athlete's primary running goal (e.g., 'run a marathon', 'get faster at 5k', 'general fitness', 'complete first 10k')."
        },
        race_distance: {
          type: "string" as const,
          description: "The distance of their target race, if any (e.g., '10k', 'marathon', 'half marathon'). Include unit if specified. Null or empty if no specific race."
        },
        race_date: {
          type: "string" as const,
          description: "The date of their target race in YYYY-MM-DD format, if any. Null or empty if no specific race or date."
        },
        schedule_constraints: {
          type: "string" as const,
          description: "Any constraints on their training schedule (e.g., 'busy on weekends', 'can only run in mornings', 'no specific constraints'). Null if none."
        },
        injury_history: {
          type: "string" as const,
          description: "A brief description of any significant past injuries or current niggles. Null if no notable history."
        },
        shoe_size: {
          type: "string" as const,
          description: "The athlete's shoe size (e.g., 'US 9.5', 'EU 43'). Include region/system if specified."
        },
        clothing_size: {
          type: "string" as const,
          description: "The athlete's typical clothing size for running apparel (e.g., 'M', 'Large')."
        }
      },
      required: [
        "nickname",
