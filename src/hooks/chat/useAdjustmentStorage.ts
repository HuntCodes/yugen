  ...
  const applyPlanChangeToSupabase = async (
    ...
    try {
      ...
      if (isOutdatedYear) {
        ...
        console.log(
          'Detected outdated year in update request. Searching based on month/day pattern.'
        );
        ...
        if (!error && data && data.length > 0) {
          ...
          workouts = data.filter((workout) => {
            ...
            return (
              workoutDate.getMonth() === updateDate.getMonth() &&
          ...
          if (workouts.length === 0) {
            console.log(
              'No exact month/day match found. Looking for closest date match in week',
              update.week
            );
      ...
      // Add suggested location update if provided
      if (update.new_suggested_location !== undefined) {
        supabaseUpdate.suggested_location = update.new_suggested_location;
      }
      ...
      if (verifyError) {
        ...
        if (
          verifyData.notes === update.new_notes &&
          verifyData.distance === update.new_distance &&
          verifyData.time === update.new_time &&
          (update.new_suggested_location === undefined ||
            verifyData.suggested_location === update.new_suggested_location)
        ) {
