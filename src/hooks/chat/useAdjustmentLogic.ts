...
export function useAdjustmentLogic() {
  ...
  const { parseAdjustmentMessage, generatePlanAdjustment, generateConfirmationMessage } =
    useAdjustmentParser();

  const { applyPlanChangeToSupabase, testDatabasePermissions } = useAdjustmentStorage();
  ...
  const handleMessage = async (
      ...
      if (parsedMessage.type === 'confirm') {
        ...
        if (success) {
          ...
          onMessageResponse({
            sender: 'coach',
            message:
              "I'm sorry, but I couldn't update your training plan. There might be a technical issue. Let me try again later or you can try making a different change.",
          });
        }
        ...
        onMessageResponse({
          sender: 'coach',
          message:
            "Alright, I won't make any changes to your training plan. Let me know if you'd like to adjust something else or if you have any questions.",
        });
        return true;
      }
    }
    ...
    if (parsedMessage.type === 'adjust') {
      ...
      if (
        message.toLowerCase().includes('test database') ||
...
• Notes: ${adjustment.new_notes}`;
        ...
        onMessageResponse({
          sender: 'coach',
          message:
            "I'd like to help adjust your training plan, but I'm having trouble understanding exactly what change you'd like. Could you clarify which workout you want to change and how you'd like it modified?",
        });
        return true;
      }
    }
