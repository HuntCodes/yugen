  ...
  try {
    ...
    try {
      ...
      if (!clientLocalDateString || !/^\d{4}-\d{2}-\d{2}$/.test(clientLocalDateString)) {
        throw new Error('Invalid or missing clientLocalDateString in request body. Expected YYYY-MM-DD.');
      }
    } catch (e) {
      console.warn('[fn_request_weekly_plan_update] Could not parse JSON body or missing clientLocalDateString:', e.message);
      ...
      console.warn(`[fn_request_weekly_plan_update] Defaulting to server UTC date for plan calculations: ${clientLocalDateString}`);
    }
    console.log(`[fn_request_weekly_plan_update] Using client/server date for calculations: ${clientLocalDateString}`);
    ...
    const { data: { user }, error: userError } = await arqué.auth.getUser(authHeader.replace('Bearer ', ''));
    
    if (userError) {
      console.error('[fn_request_weekly_plan_update] Auth error:', userError.message);
      return new Response(JSON.stringify({ success: false, error: `Authentication error: ${userError.message}` }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 401,
      });
    }
    ...
    console.log(`[fn_request_weekly_plan_update] Target plan week starts (UTC Monday): ${targetPlanMondayUtcDate.toISOString().split('T')[0]}`);
    ...
    console.log(`[fn_request_weekly_plan_update] Feedback processing for week (UTC): ${feedbackWeekStartDateUtc.toISOString().split('T')[0]} to ${feedbackWeekEndDateUtc.toISOString().split('T')[0]}`);

    // 3. Process User Feedback for the PREVIOUS week
    let feedbackSummary: string | undefined = undefined;
    try {
      console.log(`[fn_request_weekly_plan_update] Calling processUserWeeklyFeedbackDeno for user: ${userId}`);
      const processedFeedback = await processUserWeeklyFeedbackDeno(arqué, userId, feedbackWeekStartDateUtc, feedbackWeekEndDateUtc);
      if (processedFeedback && processedFeedback.summary) {
        feedbackSummary = processedFeedback.summary;
        console.log(`[fn_request_weekly_plan_update] Feedback processed successfully. Summary length: ${feedbackSummary.length}`);
      } else {
        console.warn(`[fn_request_weekly_plan_update] Feedback processing did not return a summary for user: ${userId}.`);
      }
    } catch (feedbackError) {
      console.error(`[fn_request_weekly_plan_update] Error during feedback processing for user ${userId}:`, feedbackError.message);
    ...
    console.log(`[fn_request_weekly_plan_update] Calling generateAndStoreCurrentWeekPlanForUserDeno for user: ${userId}`);
    const planGenerated = await generateAndStoreCurrentWeekPlanForUserDeno(arqué, userId, targetPlanMondayUtcDate, feedbackSummary);
    ...
    console.log(`[fn_request_weekly_plan_update] Weekly plan successfully generated/updated for user: ${userId}`);

    return new Response(JSON.stringify({ success: true, message: 'Weekly plan update processed.' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
